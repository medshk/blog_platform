...
console.log("apiiiiiiiiiiiiiiiiii", process.env.REACT_APP_API_KEY);
...
export const uploadImage = (image) => {
  ...
  const url = new Promise((resolve, reject) => {
    ...
    const storageRef = ref(
      storage,
      `/images/${Date.now().toString()}${image.name}`
    );
...
export const getAuthors = async () => {
  ...
  const res = await getDocs(colRef);
  const authors = res.docs.map((doc) => ({
    id: doc.id,
    ...doc.data(),
  }));
...
export const createAuthor = (author) => {
  const colRef = collection(db, "authors");
  setDoc(doc(colRef, author.authorId), author).then(() => {
