...
const ChangePassword = () => {
  ...
  const submit = (data) => {
    signInWithEmailAndPassword(auth, user.email, data.currentPassword)
      .then((cred) => {
        setError("");
          ...
          updatePassword(user, data.newPassword)
            ...
            .catch((error) => {
              ...
              if (error.message.toString().includes("recent-login")) {
                setError({
                  confirmPassword: "this action requires fresh login",
                });
              }
            });
        }
  ...
  return (
    <form onSubmit={handleSubmit(submit)}>
      ...
      <button
        ...
        disabled={!isValid}
        className="btn"
        style={{ marginLeft: "auto", height: "2.3rem", opacity: 0.3 }}
      >
        Save Changes
      </button>
